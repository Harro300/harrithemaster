...
const JanisolParioviCalculator: React.FC = () => {
  const { colors, theme } = useTheme()
  const [kayntiovenLeveys, setKayntiovenLeveys] = useState<string>('')
  const [lisaovenLeveys, setLisaovenLeveys] = useState<string>('')
  ...
  const calculate = () => {
    ...
    const lisaoviLeveys = parseFloat(lisaovenLeveys)
    const potkupellinKorkeusNum = parseFloat(potkupellinKorkeus)

    if (!kayntioviLeveys || !lisaoviLeveys || !potkupellinKorkeusNum) {
    ...
    const lisaovenUretaaniKorkeus = potkupellinKorkeusNum - 126
    const lisaovenUretaaniLeveys = lisaoviLeveys + 46
    ...
    // Potkupelti - Lisäovi
    const lisaovenSisapeltiKorkeus = potkupellinKorkeusNum - 65 + sisapeltiKorkeusAdjustment + heightAdjustment
    const lisaovenSisapeltiLeveys = lisaoviLeveys + 140
    const lisaovenUlkopeltiKorkeus = potkupellinKorkeusNum - 16 + ulkopeltiKorkeusAdjustment + heightAdjustment
    const lisaovenUlkopeltiLeveys = lisaoviLeveys + 140 + extraNarrowAdjustment

    // Harjalista
    const kayntiovenHarjalista = kayntioviLeveys + 141
    const lisaovenHarjalista = lisaoviLeveys + 141

    // Lasilista
    // Pariovissa: jokaisessa ruudussa on 4 pystylistaa (käyntioven 2 + lisäoven 2)
    const lasilistaResults: Array<{ pystylista: number; vaakalista: number }> = []
    
    // Käyntioven vaakalista
    const kayntiovenVaakalista = kayntioviLeveys + 3
    // Lisäoven vaakalista
    const lisaovenVaakalista = lisaoviLeveys + 3
    
    if (numberOfPanes === 1) {
      // Default: use first pane height or potkupellin korkeus if not specified
      const ruudunKorkeus = parseFloat(ruudunKorkeudet[1] || potkupellinKorkeus)
      if (ruudunKorkeus) {
        const pystylistaKorkeus = ruudunKorkeus + 41
        // Pariovissa: käyntioven 2 pystylistaa + lisäoven 2 pystylistaa = 4 yhteensä
        lasilistaResults.push({
          pystylista: pystylistaKorkeus,
          vaakalista: kayntiovenVaakalista
        })
        // Lisätään sama pystylista vielä kerran (yhteensä 4 kpl)
        lasilistaResults.push({
          pystylista: pystylistaKorkeus,
          vaakalista: kayntiovenVaakalista
        })
      ...
      // Multiple panes: use individual heights
    ...
    setResults({
      ...
      lisaovenVaakalista: lisaovenVaakalista,
      uretaani: [
        { korkeus: kayntiovenUretaaniKorkeus, leveys: kayntiovenUretaaniLeveys },
        { korkeus: lisaovenUretaaniKorkeus, leveys: lisaovenUretaaniLeveys }
      ],
      potkupelti: [
        ...
        { korkeus: lisaovenSisapeltiKorkeus, leveys: lisaovenSisapeltiLeveys },
        { korkeus: lisaovenUlkopeltiKorkeus, leveys: lisaovenUlkopeltiLeveys }
      ],
      harjalista: [kayntiovenHarjalista, lisaovenHarjalista]
    })
  }
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
        <h2 style={{ color: colors.primary, fontSize: '1.8rem', fontWeight: '700' }}>Janisol Pariovi</h2>
      ...
      <SettingsModal
        ...
        onNumberOfPanesChange={(panes) => {
          ...
          // Clear ruudun korkeudet when changing number of panes
          setRuudunKorkeudet({})
        }}
      />
      ...
      <SaveLoadModal
        ...
        onLoad={(calc: SavedCalculation) => {
          ...
          setLisaovenLeveys(calc.inputs.lisaovenLeveys || '')
        ...
        currentCalculation={results ? {
          calculatorType: 'Janisol Pariovi',
          inputs: {
            kayntiovenLeveys,
            lisaovenLeveys,
            potkupellinKorkeus,
      ...
      <div style={{ display: 'grid', gap: '20px', marginBottom: '20px' }}>
        ...
        <div>
          ...
          <input
            type="number"
            value={lisaovenLeveys}
            onChange={(e) => setLisaovenLeveys(e.target.value)}
            style={inputStyle}
            onFocus={(e) => {
              e.currentTarget.style.borderColor = colors.primary
              e.currentTarget.style.boxShadow = `0 0 0 3px ${colors.primary}33`
                ...
                {results.lasilista.length > 0 && (
                  <>
                    <div style={{ marginTop: '5px', fontFamily: 'monospace', color: colors.text }}>
                      -{Math.round(results.lasilista[0].vaakalista)} X {results.lasilista.length}
                    </div>
                    {results.lisaovenVaakalista && (
                      <div style={{ marginTop: '5px', fontFamily: 'monospace', color: colors.text }}>
                        -{Math.round(results.lisaovenVaakalista)} X {results.lasilista.length}
                      </div>
                    )}
                  </>
                )}
...
export default JanisolParioviCalculator
